...
init_u1b=[u1a(end,1); u1a(end,2); u1a(end,3); u1a(end,4); u1a(end,5); u1a(end,6); u1a(end,7); mf1; ve1];
...
init_u2=[u1b(end,1); u1b(end,2); u1b(end,3); u1b(end,4); u1b(end,5); u1b(end,6); mp2+ms2+mp3+ms3+mp4+ms4+m_star; mf2; ve2];
...
init_u3=[u2(end,1); u2(end,2); u2(end,3); u2(end,4); u2(end,5); u2(end,6); mp3+ms3+ms4+mp4+m_star; mf3; ve3];
...
init_u3b=[u3(end,1); u3(end,2); u3(end,3); u3(end,4); u3(end,5); u3(end,6); u3(end,7);0;0];
[t3b,u3b]=ode45(@orbiter3D, t3b, init_u3b);

init_u3c=[u3b(end,1); u3b(end,2); u3b(end,3); u3b(end,4); u3b(end,5); u3b(end,6); u3(end,7);mf3;ve3];
...
text(u5(Ia,1),u5(Ia,2),u5(Ia,3),"Apoapsis ("+ha+" km) \rightarrow",'Color','r','HorizontalAlignment','right', 'FontSize',14,'fontweight', 'bold')
text(u5(Ip,1),u5(Ip,2),u5(Ip,3),"\leftarrow Periapsis ("+hp+" km)"','Color','r','HorizontalAlignment','left','FontSize',14,'fontweight', 'bold')
disp("Perigee : "+hp+" km")
disp("Apogee  : "+ha+" km")
